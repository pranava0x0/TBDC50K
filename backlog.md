...
- [ ] **DRY map-layer toggle pattern in tools.js** — Extract the repeated toggle-on/off pattern (check flag → remove layer → update button text/class, or add layer → update button) used by bathrooms, spectator spots, block parties, alt routes, course sections, and heatmap into a shared `toggleMapLayer()` helper. ~80 lines of near-identical code across 6 features. Note: each toggle has different async/sync "create" logic, so the helper handles only the toggle-off + button state, with a callback for the "on" path.
- [ ] **Split tools.js into focused modules** — At 1146 lines (28% of all JS), `tools.js` is a grab bag: weather, audio, bathrooms, spectator spots, block parties, alt routes, heatmap, course sections, passport, view toggle, race card, offline caching, GPX export, calendar export. Split into `weather.js` (~50 lines), `map-layers.js` (bathrooms + spectators + heatmap + alt routes, ~400 lines), and keep the rest in `tools.js` (~700 lines). Reduces cognitive load and makes it clearer where to add new features. Prerequisite: event delegation (removes the 31 `window.*` shims).
- [ ] **Extract static data from tools.js** — Move `SPECTATOR_SPOTS`, `ALT_ROUTES`, `FINISHER_DATA`, `SEGMENT_RECORDS`, `SAUCE_COPY` into a `data.js` constants file. These are pure data (~200 lines) that clutter the logic. Makes tools.js purely functional and data independently editable.
